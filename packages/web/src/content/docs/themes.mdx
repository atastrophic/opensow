...
With OpenSow you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, OpenSow uses our own `opensow` theme.
...
OpenSow comes with several built-in themes.
...
- Want OpenSow to match their terminal's appearance
...
OpenSow supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/opensow/themes/*.json` or `$XDG_CONFIG_HOME/opensow/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.opensow/themes/*.json`
4. **Current working directory** - Defined in `./.opensow/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/opensow/themes
vim ~/.config/opensow/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .opensow/themes
vim .opensow/themes/my-theme.json
```
...
{
  "$schema": "https://opensow.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
