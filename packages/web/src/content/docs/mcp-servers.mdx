...
You can add external tools to OpenSow using the _Model Context Protocol_, or MCP. OpenSow supports both local and remote servers.
...
You can define MCP servers in your [OpenSow Config](https://opensow.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="opensow.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/opensow` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
OpenSow automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, OpenSow will:
...
If the server requires authentication, OpenSow will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `opensow mcp auth <server-name>`.
...
```bash
opensow mcp auth my-oauth-server
```
...
```bash
opensow mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, OpenSow will store the tokens securely in `~/.local/share/opensow/mcp-auth.json`.
...
opensow mcp debug my-oauth-server
```
...
Your MCPs are available as tools in OpenSow, alongside built-in tools. So you can manage them through the OpenSow config like any other tool.
...
```json title="opensow.json" {11, 14-18}
{
  "$schema": "https://opensow.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
opensow mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect OpenSow to your Sentry account.
