---
title: TUI
description: Using the OpenSow terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenSow provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running OpenSow starts the TUI for the current directory.
...
When using the OpenSow TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to OpenSow. Allows you to select from available providers and add their API keys.
...
Run a shell command using the current session context.
...
You can customize TUI behavior through your OpenSow config file.
