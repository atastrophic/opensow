---
title: Providers
description: Using any LLM provider in OpenSow.
---

import config from "../../../config.mjs"
export const console = config.console

OpenSow uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your OpenSow config.
...
in `~/.local/share/opensow/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your OpenSow
config.
...
OpenSow Zen is a list of models provided by the OpenSow team that have been
tested and verified to work well with OpenSow. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with OpenSow Zen.
:::

1. Run the `/connect` command in the TUI, select opensow, and head to [opensow.ai/auth](https://opensow.ai/auth).
...
It works like any other provider in OpenSow and is completely optional to use.
...
To use Amazon Bedrock with OpenSow:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running opensow:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY opensow

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile opensow

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX opensow
   ```
   ...
   For project-specific or persistent configuration, use `opensow.json`:

   ```json title="opensow.json"
   {
     "$schema": "https://opensow.ai/config.json",
     "provider": {
       "amazon-bedrock": {
...
:::info
Using your Claude Pro/Max subscription in OpenSow is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for opensow to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX opensow
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX opensow
   ```
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your opensow config.
...
to store token in opensow auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
OpenSow uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock OpenSow
to only use your own GitLab-hosted instance, add the following to your
`opensow.json` file. It is also recommended to disable session sharing.
...
More documentation on [opensow-gitlab-auth](https://www.npmjs.com/package/@gitlab/opensow-gitlab-auth) homepage.

##### Configuration

Customize through `opensow.json`:
...
{
  "$schema": "https://opensow.ai/config.json",
  "plugin": ["@gitlab/opensow-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with opensow:
...
To use Google Vertex AI with OpenSow:
...
2. Set the required environment variables:
   ...
   Set them while running opensow.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id opensow
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through opensow, you can always configure it yourself.
...
```jsonc title="~/.config/opensow/opensow.jsonc"
{
  "$schema": "https://opensow.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opensow-helicone-session](https://github.com/H2Shami/opensow-helicone-session) plugin to automatically log each OpenSow conversation as a session in Helicone.

```bash
npm install -g opensow-helicone-session
```
...
```json title="opensow.json"
{
  "plugin": ["opensow-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each OpenSow conversation listed as a separate session.
...
You can configure opensow to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="opensow.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://opensow.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure opensow to use local models through LM Studio.

```json title="opensow.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://opensow.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure opensow to use local models through Ollama.

```json title="opensow.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://opensow.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with OpenSow:
...
3. Copy the API key for use in OpenSow.
...
6. **Important**: Before using cloud models in OpenSow, you must pull the model information locally:
...
OpenSow Zen is a list of tested and verified models provided by the OpenSow team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>OpenSow Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **OpenSow Zen**.

   ```txt
   /connect
   ```

3. Enter your OpenSow API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your opensow config.
...
5. You can also customize them through your opensow config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' opensow
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group opensow
   ```
...
You can also customize models through your opensow config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with opensow. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in opensow.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `opensow.json` file in your project directory:

   ```json title="opensow.json" ""myprovider"" {5-15}
   {
     "$schema": "https://opensow.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="opensow.json" {9,11,17-20}
{
  "$schema": "https://opensow.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow OpenSow to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `opensow auth list` to see if the credentials
...
2. For custom providers, check the opensow config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your opensow config.
