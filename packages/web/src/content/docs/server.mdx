---
title: Server
description: Interact with opensow server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `opensow serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an opensow client can use.
...
```bash
opensow serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
opensow serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `opensow`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `opensow serve` and `opensow web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password opensow serve
```
...
When you run `opensow` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the opensow server to interact with opensow programmatically.
:::

This architecture lets opensow support multiple clients and allows you to interact with opensow programmatically.

You can run `opensow serve` to start a standalone server. If you have the
opensow TUI running, `opensow serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the OpenSow [IDE](/docs/ide) plugins.
...
The opensow server exposes the following APIs.
