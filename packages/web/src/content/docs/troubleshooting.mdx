...
To debug issues with OpenSow, start by checking the logs and local data it stores on disk.
...
- **macOS/Linux**: `~/.local/share/opensow/log/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\opensow\log`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `opensow --log-level DEBUG`.

---

## Storage

opensow stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/opensow/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\opensow`
...
OpenSow Desktop runs a local OpenSow server (the `opensow-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `OpenSow` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.config/opensow/opensow.jsonc` (or `~/.config/opensow/opensow.json`)
- **macOS/Linux** (older installs): `~/.local/share/opensow/opensow.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\opensow\opensow.jsonc`
...
OpenSow can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:

- **Global plugins**
  - **macOS/Linux**: `~/.config/opensow/plugins/`
  - **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\opensow\plugins`
- **Project plugins** (only if you use per-project config)
  - `<your-project>/.opensow/plugins/`
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so OpenSow can rebuild it.

1. Quit OpenSow Desktop completely.
2. Delete the cache directory:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/opensow`
- **Linux**: delete `~/.cache/opensow` (or run `rm -rf ~/.cache/opensow`)
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\opensow`

3. Restart OpenSow Desktop.

---

### Fix server connection issues

OpenSow Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If your `opensow.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.
...
On Windows, OpenSow Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Notifications not showing

OpenSow Desktop only shows system notifications when:

- notifications are enabled for OpenSow in your OS settings, and
...
1. Quit OpenSow Desktop.
2. Find and delete these files (they live in the OpenSow Desktop app data directory):

- `opensow.settings.dat` (desktop default server URL)
- `opensow.global.dat` and `opensow.workspace.*.dat` (UI state like recent servers/projects)
...
If you're experiencing issues with OpenSow:

1. **Report issues on GitHub**

   The best way to report bugs or request features is through our GitHub repository:

   [**github.com/atasrophic/opensow/issues**](https://github.com/atasrophic/opensow/issues)
...
2. **Join our Discord**
   ...
   [**opensow.ai/discord**](https://opensow.ai/discord)
...
3. Ensure you have the latest version with `opensow upgrade`
...
To figure out what models you have access to, run `opensow models`
...
2. If the issue persists, try clearing your stored configuration:
   ...
   ```bash
   rm -rf ~/.local/share/opensow
   ```

   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.local\share\opensow`
...
If you encounter API call errors, this may be due to outdated provider packages. opensow dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
1. Clear the provider package cache:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.cache\opensow`

2. Restart opensow to reinstall the latest provider packages

This will force opensow to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
opensow will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
