---
title: Custom Tools
description: Create tools the LLM can call in opensow.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside opensow's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.opensow/tools/` directory of your project.
- Or globally, by placing them in `~/.config/opensow/tools/`.
...
```ts title=".opensow/tools/database.ts" {1}
...
```ts title=".opensow/tools/math.ts"
import { tool } from "@opensow-ai/plugin"
...
```ts title=".opensow/tools/project.ts" {8}
import { tool } from "@opensow-ai/plugin"
...
```python title=".opensow/tools/add.py"
import sys
...
```ts title=".opensow/tools/python-add.ts" {10}
import { tool } from "@opensow-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.directory, ".opensow/tools/add.py")
