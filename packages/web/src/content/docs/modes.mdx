...
:::caution
Modes are now configured through the `agent` option in the opensow config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in opensow allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the opensow config.
...
opensow comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.opensow/plans/*.md` to detail the plan itself
...
Configure modes in your `opensow.json` config file:
...
- Global: `~/.config/opensow/modes/`
- Project: `.opensow/modes/`

```markdown title="~/.config/opensow/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
both the global opensow config and the project specific config.
...
Create mode files in `.opensow/modes/` for project-specific modes or `~/.config/opensow/modes/` for global modes:

```markdown title=".opensow/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/opensow/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
