...
There are two types of agents in OpenSow; primary agents and subagents.
...
OpenSow comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
OpenSow comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

OpenSow comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `opensow.json` config file:
...
- Global: `~/.config/opensow/agents/`
- Per-project: `.opensow/agents/`

```markdown title="~/.config/opensow/agents/review.md"
---
...
If no temperature is specified, OpenSow uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global OpenSow config and the project specific config.
...
The model ID in your OpenSow config uses the format `provider/model-id`. For example, if you're using [OpenSow Zen](/docs/zen), you would use `opensow/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="opensow.json" {3-6,9-12}
{
  "$schema": "https://opensow.ai/config.json",
  "tools": {
    "write": true,
...
```json title="opensow.json" {3-5,8-10}
{
  "$schema": "https://opensow.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Run `opensow models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/atasrophic/opensow).
:::
...
```markdown title="~/.config/opensow/agents/docs-writer.md"
---
...
```markdown title="~/.config/opensow/agents/security-auditor.md"
---
