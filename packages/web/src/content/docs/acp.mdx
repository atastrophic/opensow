---
title: ACP Support
description: Use OpenSow in any ACP-compatible editor.
---

OpenSow supports the [Agent Client Protocol](https://agentclientprotocol.com) or (ACP), allowing you to use it directly in compatible editors and IDEs.
...
To use OpenSow via ACP, configure your editor to run the `opensow acp` command.

The command starts OpenSow as an ACP-compatible subprocess that communicates with your editor over JSON-RPC via stdio.
...
```json title="acp.json"
{
  "agent_servers": {
    "OpenSow": {
      "command": "/absolute/path/bin/opensow",
      "args": ["acp"]
    }
  }
}
...
To open it, use the new 'OpenSow' agent in the AI Chat agent selector.
...
To use OpenSow as an ACP agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), add the following to your Neovim config:
...
This config sets up CodeCompanion to use OpenSow as the ACP agent for chat.
...
OpenSow works the same via ACP as it does in the terminal. All features are supported:
...
- MCP servers configured in your OpenSow config
