...
description: What's new in OpenSow 1.0.
---

OpenSow 1.0 is a complete rewrite of the TUI.
...
The new TUI works like the old one since it connects to the same opensow server.
...
version. However some older versions of OpenSow always grab latest.

To upgrade manually, run

```bash
$ opensow upgrade 1.0.0
```

To downgrade back to 0.x, run

```bash
$ opensow upgrade 0.15.31
```
