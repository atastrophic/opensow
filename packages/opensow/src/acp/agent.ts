...
import type { Event, OpencodeClient, SessionMessageResponse } from "@opensow-ai/sdk/v2"
...
export namespace ACP {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `opensow auth login` in the terminal",
        name: "Login with opensow",
        id: "opensow-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // OpenSow stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const opensowProvider = providers.find((p) => p.id === "opensow")
    if (opensowProvider) {
      if (opensowProvider.models["big-pickle"]) {
        return { providerID: "opensow", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(opensowProvider.models))
      if (best) {
        return {
