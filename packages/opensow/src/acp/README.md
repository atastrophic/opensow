...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for opensow.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal opensow sessions
...
opensow acp --cwd /path/to/project
```
...
- **Terminal Integration**: Full terminal support via opensow's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | opensow acp
```
...
ACP sessions map cleanly to opensow's internal session model:
