...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall opensow and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall OpenSow")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g opensow-ai",
      pnpm: "pnpm uninstall -g opensow-ai",
      bun: "bun remove -g opensow-ai",
      yarn: "yarn global remove opensow-ai",
      brew: "brew uninstall opensow",
      choco: "choco uninstall opensow",
      scoop: "scoop uninstall opensow",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall opensow -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using OpenSow!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# opensow") || content.includes(".opensow/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".opensow/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".opensow/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".opensow"))
    ) {
      continue
    }
