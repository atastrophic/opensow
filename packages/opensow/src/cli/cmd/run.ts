...
import { createOpencodeClient, type OpencodeClient } from "@opensow-ai/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run opensow with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running opensow server (e.g., http://localhost:4096)",
      })
      .option("port", {
        type: "number",
  ...
  handler: async (args) => {
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createOpencodeClient({ baseUrl: "http://opensow.internal", fetch: fetchFn })
