...
export function useConnected() {
  const sync = useSync()
  return createMemo(() =>
    sync.data.provider.some((x) => x.id !== "opensow" || Object.values(x.models).some((y) => y.cost?.input !== 0)),
  )
}
          ...
          return [
            {
              ...
              category: "Favorites",
              disabled: provider.id === "opensow" && model.id.includes("-nano"),
              footer: model.cost?.input === 0 && provider.id === "opensow" ? "Free" : undefined,
              onSelect: () => {
                dialog.clear()
                local.model.set(
                  {
        ...
        pipe(
          ...
          map(([model, info]) => {
            ...
            return {
              ...
              disabled: provider.id === "opensow" && model.includes("-nano"),
              footer: info.cost?.input === 0 && provider.id === "opensow" ? "Free" : undefined,
              onSelect() {
                dialog.clear()
                local.model.set(
                  {
