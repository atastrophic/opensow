...
const TIPS = [
  ...
  "Add {highlight}$schema{/highlight} to your config for autocomplete when editing settings",
  ...
  "Use backticks in commands to inject shell output (e.g., {highlight}`date`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.opensow/agent/{/highlight} for specialized gardener personas",
  "Configure per-gardener permissions for {highlight}bash{/highlight} and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"weather *": "allow"{/highlight} for granular tool permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to prevent destructive commands',
  'Configure {highlight}"soil_test": "ask"{/highlight} to require approval before running analyses',
  "OpenSow auto-formats output for clean, readable garden plans",
  'Set {highlight}"formatter": false{/highlight} in config to disable all auto-formatting',
  "Define custom output formats in config for different garden report styles",
  "OpenSow uses smart analysis for intelligent garden recommendations",
  "Create files in {highlight}.opensow/tool/{/highlight} to forge new garden tools",
  "Tool definitions can call external scripts and APIs",
  "Add files to {highlight}.opensow/plugin/{/highlight} for event hooks",
  ...
  "Use {highlight}opensow run -f notes.md{/highlight} to attach reference files via CLI",
  ...
  "Use {highlight}/schedule{/highlight} to see upcoming planting windows for your zone",
  "Ask about companion planting to maximize your garden's yield",
  "Use {highlight}/review{/highlight} to get feedback on your season plan — timing, spacing, and companions",
  "Ask about frost dates to plan your seed-starting schedule",
  ...
  "Run {highlight}opensow{/highlight} in a container for portable gardening setups",
  "Use {highlight}/connect{/highlight} with OpenSow Zen for curated, tested models",
  "Commit your plot's {highlight}AGENTS.md{/highlight} file for team sharing",
  "Run {highlight}/review crops{/highlight} to check companion planting and spacing",
