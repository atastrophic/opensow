...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run opensow",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for opensow session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found opensow session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`opensow import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting opensow...")
        UI.println()

        // Launch opensow TUI with session ID if available
        const { spawn } = await import("child_process")
        const opensowArgs = sessionId ? ["-s", sessionId] : []
        const opensowProcess = spawn("opensow", opensowArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          opensowProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`opensow exited with code ${code}`))
          })
          opensowProcess.on("error", reject)
        })
      },
    })
  },
