...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    async opensow(input) {
      const hasKey = await (async () => {
        ...
        if (config.provider?.["opensow"]?.options?.apiKey) return true
        return false
      })()
    ...
    "amazon-bedrock": async () => {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from opensow.json provider config
    ...
    cerebras: async () => {
      return {
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "opensow",
          },
        },
      }
    },
  ...
  export async function getSmallModel(providerID: string) {
    ...
    if (provider) {
      ...
      if (providerID.startsWith("opensow")) {
        priority = ["gpt-5-nano"]
      }
    ...
    // Check if opensow provider is available before using it
    const opensowProvider = await state().then((state) => state.providers["opensow"])
    if (opensowProvider && opensowProvider.models["gpt-5-nano"]) {
      return getModel("opensow", "gpt-5-nano")
    }
