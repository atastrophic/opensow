  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Bun.file(path.join(tmp.path, "opensow.json")).text()
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "opensow.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .opensow directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opensowDir = path.join(dir, ".opensow")
      await fs.mkdir(opensowDir, { recursive: true })
      const agentDir = path.join(opensowDir, "agent")
...
test("loads agents from .opensow/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opensowDir = path.join(dir, ".opensow")
      await fs.mkdir(opensowDir, { recursive: true })

      const agentsDir = path.join(opensowDir, "agents")
...
test("loads commands from .opensow/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opensowDir = path.join(dir, ".opensow")
      await fs.mkdir(opensowDir, { recursive: true })

      const commandDir = path.join(opensowDir, "command")
...
test("loads commands from .opensow/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opensowDir = path.join(dir, ".opensow")
      await fs.mkdir(opensowDir, { recursive: true })

      const commandsDir = path.join(opensowDir, "commands")
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(
        path.join(dir, "opensow.json"),
        JSON.stringify({ $schema: "https://opensow.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "opensow.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .opensow config
      const projectDir = path.join(dir, "project")
      const opensowDir = path.join(projectDir, ".opensow")
      await fs.mkdir(opensowDir, { recursive: true })
      ...
      // Local .opensow config with different plugins
      await Bun.write(
        path.join(opensowDir, "opensow.json"),
        JSON.stringify({
          $schema: "https://opensow.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .opensow config with some overlapping plugins
      await Bun.write(
        path.join(opensowDir, "opensow.json"),
        JSON.stringify({
          $schema: "https://opensow.ai/config.json",
...
test("local .opensow config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .opensow directory config enables it
      const opensowDir = path.join(dir, ".opensow")
      await fs.mkdir(opensowDir, { recursive: true })
      await Bun.write(
        path.join(opensowDir, "opensow.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/opensow")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/opensow")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-opensow@2.4.3")).toBe("oh-my-opensow")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@opensow/plugin@2.0.0")).toBe("@opensow/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-opensow@2.4.3", "file:///project/.opensow/plugin/oh-my-opensow.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.opensow/plugin/oh-my-opensow.js")
  })
  ...
  test("local plugin directory overrides global opensow.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const opensowDir = path.join(projectDir, ".opensow")
        const pluginDir = path.join(opensowDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("OPENCODE_DISABLE_PROJECT_CONFIG", () => {
  ...
  test("skips project .opensow/ directories when flag is set", async () => {
    const originalEnv = process.env["OPENCODE_DISABLE_PROJECT_CONFIG"]
    process.env["OPENCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .opensow directory with a command
          const opensowDir = path.join(dir, ".opensow", "command")
          await fs.mkdir(opensowDir, { recursive: true })
          await Bun.write(path.join(opensowDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .opensow should NOT be in directories list
