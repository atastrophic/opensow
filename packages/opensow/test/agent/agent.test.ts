...
test("returns default native agents when no config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(names).toContain("gartenmeister")
...
test("gartenmeister agent has correct default properties", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      const build = await Agent.get("gartenmeister")
...
test("research agent denies edits except .opensow/research/*.md", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      ...
      expect(PermissionNext.evaluate("edit", ".opensow/research/foo.md", research!.permission).action).toBe("allow")
    },
  })
})
...
test("custom agent config overrides native agent properties", async () => {
    ...
    config: {
      agent: {
        gartenmeister: {
          ...
          description: "Custom gartenmeister agent",
          temperature: 0.7,
          color: "#FF0000",
        },
      },
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(build?.description).toBe("Custom gartenmeister agent")
...
test("agent steps/maxSteps config sets steps property", async () => {
  await using tmp = await tmpdir({
    config: {
      agent: {
        gartenmeister: { steps: 50 },
        research: { maxSteps: 100 },
      },
    },
  })
  await Instance.provide({
...
test("agent prompt can be set from config", async () => {
  await using tmp = await tmpdir({
    config: {
      agent: {
        gartenmeister: { prompt: "Custom system prompt" },
      },
    },
  })
  await Instance.provide({
...
test("defaultAgent returns gartenmeister when no default_agent config", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      const agent = await Agent.defaultAgent()
      expect(agent).toBe("gartenmeister")
    },
  })
})

test("defaultAgent respects default_agent config set to research", async () => {
  await using tmp = await tmpdir({
    config: {
...
test("defaultAgent returns research when gartenmeister is disabled and default_agent not set", async () => {
  await using tmp = await tmpdir({
    config: {
      agent: {
        gartenmeister: { disable: true },
      },
    },
  })
  await Instance.provide({
    ...
    fn: async () => {
      const agent = await Agent.defaultAgent()
      // gartenmeister is disabled, so it should return research (next primary agent)
      expect(agent).toBe("research")
    },
  })
})
...
test("defaultAgent throws when all primary agents are disabled", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      // gartenmeister and research are disabled, no primary-capable agents remain
