...
import { createOpencode, type ToolPart } from "@opensow-ai/sdk"
...
console.log("🚀 Starting opensow server...")
const opensow = await createOpencode({
  port: 0,
})
...
;(async () => {
  const events = await opensow.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new opensow session...")
    const { client, server } = opensow
    ...
    console.log("✅ Created opensow session:", createResult.data.id)
  ...
  console.log("📝 Sending to opensow:", message.text)
